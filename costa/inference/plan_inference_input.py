...
def main():
    ...
    # parser.add_argument("-m", "--mask_folder", help="Brain mask folder, used to remove non-brain regions")
    # parser.add_argument("-o", "--output_folder", help="Output folder, if None: generate new folder"
    #                                                   "along with input folder", default=None)
    parser.add_argument("-c", "--costa_dir", type=bool, help="The costa's input folder", default=True)

    args = parser.parse_args()
    input_folder = args.input_folder
    is_plan_costa_dir = args.costa_dir

    input_folder = os.path.abspath(input_folder)
    if input_folder is None or not os.path.exists(input_folder):
        raise Exception("Please specify the input folder")

    output_folder = join(os.path.dirname(input_folder), os.path.basename(input_folder) + "_normed")
    ...
    if is_plan_costa_dir:
        print("\nPlan the COSTA input folder")
        plan_the_costa_input_dir(raw_dir=input_folder, normed_dir=output_folder)
