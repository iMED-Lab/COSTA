...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        # in order to reduce the info loss caused by resampling while ensuring the homogeneity
        # version 1
        delta = 1. / 3.
        left_delta = 1 - delta
        right_delta = 1 + delta
        dynamic_spacing = [ori if (left_delta * tgt <= ori <= right_delta * tgt) else (
            left_delta * tgt if ori < left_delta * tgt else right_delta * tgt) for ori, tgt in
                           zip(original_spacing_transposed, target_spacing)]
        target_spacing = dynamic_spacing
        # ############# END #################################################################################

        data, seg = resample_patient(data, seg, np.array(original_spacing_transposed), np.array(target_spacing),
